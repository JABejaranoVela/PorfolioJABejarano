...
El portfolio está desplegado en un VPS de Hostinger con Ubuntu 24.04 LTS y Nginx. Está disponible públicamente en [https://jabejarano.tech](https://jabejarano.tech), y Nginx sirve la release activa desde `/var/www/PorfolioJABejarano/current/dist`.

GitHub Actions compila el proyecto con la versión de Node fijada en `.nvmrc`, instala dependencias de forma reproducible con `npm ci` y genera `dist`. El VPS solo recibe el artefacto estático, lo almacena en una carpeta de `releases` y cambia el symlink `current` de forma atómica. Cada release publica `release.json` para comprobar por HTTPS el commit que está sirviendo producción.

El proceso de migración, los secrets necesarios, las comprobaciones y el rollback están documentados en [Despliegue del portfolio en un VPS Ubuntu](docs/deployment-vps.md).
